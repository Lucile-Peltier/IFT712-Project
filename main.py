...
def main():
    ...
    [types, x_tr, id_tr, t_tr, x_ts, id_ts] = g_donnees.lecture_donnees(train_data,test_data)
    ...
    # Prédictions pour les ensembles d'entraînement et de validation
    predict_tr = np.array([classif.predict(x) for x in x_tr])
    sc_tr = classif.precision(x_tr, predict_tr)
    
    predict_ts = np.array([classif.predict(x) for x in x_ts])
